 ...
 *
 * <p>
 * This configuration defines the application's security model:
 *
 * <ul>
 * <li>Enables Spring Security web protection and method-level security.
 * <li>Configures the application as an OAuth2 Resource Server using JWT Bearer
 * tokens.
 * <li>Disables CSRF protection because the API is stateless and does not rely
 ...
 *
 * <p>
 * The JWT token is expected to contain authorization information through the
 * following claims:
 *
 ...
 * prefix, allowing authorization rules such as:
 *
 * <pre>{@code @PreAuthorize("hasRole('ADMIN')") }</pre>
 *
 * <p>
 ...
 * filter chain and method-level authorization through
        ...
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeHttpRequests(auth -> auth.requestMatchers(OPTIONS, "/**").permitAll()
                        .requestMatchers(GET, "/figurines/**", "/catalogs/{catalogType}/**", "/anniversaries/**",
                                "/demos/**", "/figurine-stores/**", "/actuator/**", "/swagger-ui.html",
                                "/swagger-ui/**", "/swagger.yaml", "/v3/api-docs/**", "/v3/api-docs.yaml")
                        .permitAll().requestMatchers(POST, "/collectors/auth/{provider}/**", "/figurine-stores/**")
