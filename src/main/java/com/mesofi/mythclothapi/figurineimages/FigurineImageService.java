 ...
 *
 * <p>
 * Images are stored as normalized URL strings and are treated as sub-resources
 * of the {@link Figurine} aggregate.
 *
 * <p>
 * All write operations are transactional to ensure consistency between
 * figurines and their associated images.
 */
@Slf4j
@Service
@Validated
...
public class FigurineImageService {
     ...
     *
     * @param request
     *            validated image creation request
     ...
     *             if the image URL already exists
     */
    @Transactional
    public FigurineImageResp createFigurineImage(@Valid FigurineImageReq request) {
        ...
        List<String> existingImages = request.isOfficialImage()
                ? existing.getOfficialImages()
                : existing.getNonOfficialImages();
     ...
     *
     * <p>
     * This method returns either official or non-official images based on the
     * provided flag.
     *
     * <p>
     * The returned collection is an immutable copy to prevent external modification
     * of the internal state.
     *
     * @param figurineId
     *            identifier of the target figurine
     * @param isOfficialImage
     *            {@code true} to retrieve official images, {@code false} for
     *            non-official images
    ...
    public FigurineImageResp retrieveFigurineImages(@Positive Long figurineId, boolean isOfficialImage) {

        var existing = findExistingFigurine(figurineId);

        return new FigurineImageResp(isOfficialImage
     ...
     * @param isOfficialImage
     *            {@code true} if the image is official, {@code false} if
     *            non-official
     * @throws FigurineNotFoundException
     *             if the figurine does not exist
     * @throws ImageNotFoundException
     *             if the image URL is not associated with the figurine
     */
    @Transactional
    public void removeFigurineImage(@Positive Long figurineId, URI imageUrl, boolean isOfficialImage) {

        var existing = findExistingFigurine(figurineId);

        List<String> images = isOfficialImage ? existing.getOfficialImages() : existing.getNonOfficialImages();
    ...
    private Figurine findExistingFigurine(Long figurineId) {
        return figurineRepository.findById(figurineId).orElseThrow(() -> new FigurineNotFoundException(figurineId));
    }
}
