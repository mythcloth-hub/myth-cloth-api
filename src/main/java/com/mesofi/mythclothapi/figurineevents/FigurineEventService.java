 ...
 *
 * <p>
 * Provides CRUD operations for events associated with a specific figurine. Each
 * event captures a dated lifecycle milestone (e.g., pre-order, release,
 * restock) linked to a
 * {@link com.mesofi.mythclothapi.figurines.model.Figurine}.
 *
 * <p>
 * All write operations validate that the referenced figurine exists before
 * persisting changes. Read operations scope queries by both figurine ID and
 * event ID to enforce ownership boundaries.
 */
@Slf4j
@Service
@Validated
...
public class FigurineEventService {
     ...
     *
     * <p>
     * Maps the incoming {@link FigurineEventReq} to a {@link FigurineEvent},
     * resolves and links the associated {@link Figurine}, and persists the entity.
     *
     * @param request
    ...
    private void linkReferences(FigurineEvent figurineEvent, long figurineId) {
        Figurine figurine = Optional.of(figurineId).flatMap(figurineRepository::findById)
     ...
     *
     * @param figurineId
     *            the ID of the figurine that owns the event
     * @param eventId
    ...
    public FigurineEventResp retrieveFigurineEvent(@NotNull Long figurineId, @NotNull Long eventId) {
        return repository.findByIdAndFigurineId(eventId, figurineId).map(mapper::toFigurineEventResp)
    ...
    public List<FigurineEventResp> retrieveFigurineEvents(@NotNull Long figurineId) {
        return Optional.of(figurineId).map(repository::findAllByFigurineId).filter(list -> !list.isEmpty())
                .orElseThrow(() -> new FigurineNotFoundException(figurineId)).stream().map(mapper::toFigurineEventResp)
                .toList();
    }
     ...
     *
     * <p>
     * Looks up the event by both {@code eventId} and {@code figurineId} to enforce
     * ownership. If the incoming request carries a different figurine ID, the event
     * is reassigned to that figurine after verifying it exists.
     *
     * @param figurineId
     *            the ID of the figurine that currently owns the event
     * @param eventId
     *            the ID of the event to update
     * @param newRequest
     *            the updated event data; must pass validation
     ...
     */
    @Transactional
    public FigurineEventResp updateFigurineEvent(@NotNull Long figurineId, @NotNull Long eventId,
            @Valid FigurineEventReq newRequest) {
        log.info("Updating event '{}' for figurine {} to [{}]({}) - {}", eventId, figurineId, newRequest.getDate(),
                newRequest.getFigurineId(), newRequest.getDescription());

        FigurineEvent figurineEvent = repository.findByIdAndFigurineId(eventId, figurineId)
                .orElseThrow(() -> new FigurineEventNotFoundException(eventId));

        // Ensure the new figurine ID exists, as the event may be reassigned to another
        // figurine.
        Figurine figurine = Optional.of(newRequest.getFigurineId()).flatMap(figurineRepository::findById)
