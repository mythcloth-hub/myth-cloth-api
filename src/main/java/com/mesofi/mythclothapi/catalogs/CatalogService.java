...
public class CatalogService {
	...
	@PostConstruct
	void init() {
		entityFactories = Map.of(CatalogType.groups, mapper::toGroup, CatalogType.series, mapper::toSeries,
				CatalogType.lineups, mapper::toLineUp, CatalogType.distributions, mapper::toDistribution);
	}
	...
	@Transactional(readOnly = true)
	public Descriptive retrieveCatalogWithDescription(@NotEmpty String catalogName, @NotNull String description) {
	...
	private Descriptive mapToEntity(String catalogName, CatalogReq request) {
		return Optional.ofNullable(entityFactories.get(CatalogType.valueOf(catalogName))).map($ -> $.apply(request))
