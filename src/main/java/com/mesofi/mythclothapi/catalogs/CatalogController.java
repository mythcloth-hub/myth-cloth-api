 ...
 *
 * <p>
 * Handles HTTP requests for CRUD operations on catalog entries, which are
 * polymorphic descriptive references organized by type (e.g., Groups, Series,
 * LineUps, Distributions). All catalog operations are routed through the
 * {@link CatalogService}, which uses a strategy pattern to handle different
 * catalog types dynamically.
 *
 * <p>
 * The controller uses path variables to specify the catalog type (e.g.,
 * {@code groups}, {@code
 * series}) and delegates to the service layer which performs type-specific
 * operations.
 *
 * @see CatalogService
 * @see CatalogType
...
public class CatalogController {
     ...
     *
     * <p>
     * This method accepts a catalog type (e.g., {@code groups}, {@code series},
     * {@code lineups}, {@code distributions}) as a path variable and creates a new
     * descriptive entry for that catalog. The response includes a {@code Location}
     * header pointing to the newly created resource.
     *
     * @param catalogType
     *            the type of catalog to create the entry in (e.g., {@code groups})
     * @param request
     *            the catalog request containing the description of the new entry
     * @return a {@link ResponseEntity} with HTTP 201 (Created) status, the
     *         {@code Location} header, and the created catalog entry as the
     *         response body
    ...
    public ResponseEntity<CatalogResp> createCatalog(@NotNull @Valid @PathVariable CatalogType catalogType,
        ...
        URI location = ServletUriComponentsBuilder.fromCurrentRequest() // e.g. /api/series
                .path("/{id}") // append /{id}
     ...
     *
     * <p>
     * Fetches a specific catalog entry from the repository corresponding to the
     * specified catalog type.
     *
     * @param catalogType
     *            the type of catalog (e.g., {@code groups}, {@code series})
     * @param id
     *            the unique identifier of the catalog entry to retrieve
     ...
     *
     * <p>
     * Returns the full list of descriptive entries in the requested catalog (for
     * example, all {@code groups}, {@code series}, {@code lineups}, or
     * {@code distributions}).
     *
     * @param catalogType
     ...
     *
     * <p>
     * Modifies the description of an existing catalog entry in the specified
     * catalog type. Only the {@code description} field is updated; other fields
     * remain unchanged.
     *
     * @param catalogType
     *            the type of catalog containing the entry to update
     * @param id
     *            the unique identifier of the catalog entry to update
     * @param catalogReq
     *            the request containing the new description
     * @return a {@link ResponseEntity} with HTTP 200 (OK) status and the updated
     *         catalog entry
     * @throws com.mesofi.mythclothapi.catalogs.exceptions.CatalogNotFoundException
     *             if the entry with the given ID does not exist in the specified
     *             catalog type
     ...
     *
     * <p>
     * Deletes the catalog entry identified by the given ID from the repository
     * corresponding to the catalog type.
     *
     * @param catalogType
     *            the type of catalog containing the entry to delete
     * @param id
     *            the unique identifier of the catalog entry to delete
