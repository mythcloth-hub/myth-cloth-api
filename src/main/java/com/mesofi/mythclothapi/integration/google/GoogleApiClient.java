...
/**
 * Client responsible for validating Google ID tokens against Google's tokeninfo
 * endpoint.
 */
@Slf4j
@Component
public class GoogleApiClient {
     ...
     *
     * @param idToken
     *            Google ID token to validate
     * @return token info returned by Google
     * @throws IntegrationException
     *             if Google returns an error or empty response
     */
    public GoogleTokenInfoResponse validateIdToken(String idToken) {
        GoogleTokenInfoResponse response = applyErrorHandling(restClient.get()
                .uri(uriBuilder -> uriBuilder.path("/tokeninfo").queryParam("id_token", idToken).build())
                .accept(MediaType.APPLICATION_JSON).retrieve()).body(GoogleTokenInfoResponse.class);

        if (response == null) {
            throw new IntegrationException(ServiceName.GOOGLE, HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    private RestClient.ResponseSpec applyErrorHandling(RestClient.ResponseSpec responseSpec) {
        return responseSpec.onStatus(HttpStatusCode::is4xxClientError, (request, response) -> {
            ...
            throw new IntegrationException(ServiceName.GOOGLE, response.getStatusCode().value(),
