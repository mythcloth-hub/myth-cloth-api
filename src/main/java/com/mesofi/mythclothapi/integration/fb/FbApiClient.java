 ...
 *
 * <p>
 * All communication errors are translated into {@link IntegrationException}
 * instances to provide a consistent integration error model across the
 * application.
 */
@Slf4j
@Component
public class FbApiClient {
	 ...
	 *
	 * @param fcCredentials
	 *            Facebook application credentials used for token validation.
	 */
	...
	/**
	 * Retrieves profile information for the user associated with the provided
	 ...
	 *             if the Facebook API returns an error or an invalid response.
	 */
	public FbUserInfoResponse getUserInfo(String accessToken) {
		return getFbResponse(uriBuilder -> uriBuilder.path("/me").queryParam("fields", "id,name,email,picture")
				.queryParam("access_token", accessToken).build(), FbUserInfoResponse.class);
	}
	 ...
	 *
	 * <p>
	 * The validation is performed using the application credentials configured for
	 * this service.
	 *
	 * @param accessToken
	 *            Facebook access token to validate.
	 * @return token validation details returned by Facebook.
	 * @throws IntegrationException
	 *             if the Facebook API returns an error or an invalid response.
	 */
	public FbTokenResponse validateAccessToken(String accessToken) {
		return getFbResponse(uriBuilder -> uriBuilder.path("/debug_token").queryParam("access_token", appToken())
				.queryParam("input_token", accessToken).build(), FbTokenResponse.class);
	}

	/**
	 * Executes a Facebook Graph API GET request and maps the response body to the
	 * specified type.
	 *
	 * @param uriFunction
	 *            function used to build the request URI.
	 * @param responseType
	 ...
	 * @throws IntegrationException
	 *             if the response is empty or Facebook returns an error.
	 */
	private <T> T getFbResponse(Function<UriBuilder, URI> uriFunction, Class<T> responseType) {

		T response = applyErrorHandling(restClient.get().uri(uriFunction).accept(MediaType.APPLICATION_JSON).retrieve())
				.body(responseType);
	 ...
	 *
	 * <p>
	 * Client-side and server-side errors are translated into
	 * {@link IntegrationException} instances.
	 *
	 * @param responseSpec
	 *            response specification to decorate.
	...
	private RestClient.ResponseSpec applyErrorHandling(RestClient.ResponseSpec responseSpec) {
		...
		return responseSpec.onStatus(HttpStatusCode::is4xxClientError, (request, response) -> {
			...
			if (response.getStatusCode() == HttpStatus.NOT_FOUND) {
				throw new IntegrationException(FACEBOOK, HttpStatus.NOT_FOUND, "Facebook resource not found.");
			}

			throw new IntegrationException(FACEBOOK, response.getStatusCode().value(), "Facebook API client error.");
		}).onStatus(HttpStatusCode::is5xxServerError, (request, response) -> {
			log.error("Facebook API returned server error: {}", response.getStatusCode().value());

			throw new IntegrationException(FACEBOOK, response.getStatusCode().value(), "Facebook servers are down.");
		});
	}
	 ...
	 * @throws IntegrationException
	 *             if the response is {@code null}.
	 */
	private <T> T requireResponse(T response) {
		if (response == null) {
			throw new IntegrationException(FACEBOOK, HttpStatus.INTERNAL_SERVER_ERROR,
