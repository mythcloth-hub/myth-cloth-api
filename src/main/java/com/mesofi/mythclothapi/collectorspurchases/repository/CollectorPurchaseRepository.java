 ...
 *
 * <p>
 * Provides CRUD operations for {@link CollectorPurchase} entities and custom
 * queries to retrieve purchases scoped to a specific collector.
 *
 * <p>
 * All queries enforce collector ownership by filtering purchases using the
...
public interface CollectorPurchaseRepository extends JpaRepository<CollectorPurchase, Long> {
     ...
     *
     * <p>
     * This method ensures that a purchase can only be retrieved when it belongs to
     ...
     * @param collectorId
     *            collector identifier
     * @return the purchase when found and owned by the collector, otherwise
     *         {@link Optional#empty()}
     */
    Optional<CollectorPurchase> findByIdAndCollectorId(Long id, Long collectorId);

    /**
     * Retrieves all purchases belonging to a collector ordered by most recent
     * purchase date.
     *
     * <p>
     * When multiple purchases have the same order date, results are ordered by
     * purchase identifier descending to provide a deterministic order.
     *
     * @param collectorId
     *            collector identifier
