 ...
 *
 * <p>
 * This controller exposes endpoints to create, update, retrieve, and delete
 * collector purchases together with their associated figurine line items.
 *
 * <p>
 * All operations are executed within the authenticated collector context. The
 * collector identifier is extracted from the authenticated JWT token and is
 * used to ensure users can only access their own purchase data.
 *
 * <p>
...
public class CollectorPurchaseController {
     ...
     *
     * <p>
     * The collector is obtained from the authenticated JWT token. The request
     * contains purchase metadata and the figurines included in the purchase.
     *
     * @param jwt
     *            authenticated collector token
     * @param request
     *            purchase information and line items to create
    ...
    public ResponseEntity<CollectorPurchaseSummaryLineItemResp> createSummaryLineItem(@AuthenticationPrincipal Jwt jwt,
        ...
        log.info(
                ...
                jwt.getSubject(), request.store(), request.orderNumber(), request.lineItems().size());

        CollectorPurchaseSummaryLineItemResp response = service.createSummaryLineItem(getCollectorId(jwt), request);
     ...
     *
     * <p>
     * The purchase must belong to the authenticated collector. Existing line items
     * may be replaced when changes are detected.
     *
     * @param jwt
     *            authenticated collector token
     * @param purchaseId
     *            identifier of the purchase to update
     * @param request
     *            updated purchase information and line items
    ...
    public ResponseEntity<CollectorPurchaseSummaryLineItemResp> updateSummaryLineItem(@AuthenticationPrincipal Jwt jwt,
        ...
        log.info(
                ...
                jwt.getSubject(), purchaseId, request.store(), request.orderNumber(), request.lineItems().size());

        CollectorPurchaseSummaryLineItemResp response = service.updateSummaryLineItem(getCollectorId(jwt), purchaseId,
                request);
     ...
     *
     * <p>
     * The response contains purchase summaries together with their associated
     * figurine line items.
     *
     * @param jwt
     *            authenticated collector token
     ...
     *
     * <p>
     * The purchase must belong to the authenticated collector.
     *
     * @param jwt
     *            authenticated collector token
     * @param purchaseId
     ...
     *            authenticated collector token
     * @param purchaseId
     *            identifier of the purchase to delete
    ...
    /**
     * Synchronizes figurine quantities from a specific purchase into a collector
     * collection.
     *
     * <p>
     * This operation updates the collection quantities based on the figurines
     * recorded in the purchase. The purchase remains unchanged as historical data.
     *
     * @param jwt
     *            authenticated collector token
     * @param purchaseId
     *            identifier of the purchase used as synchronization source
     * @param collectionId
     *            identifier of the target collection
     * @return HTTP {@code 202 Accepted} when synchronization is requested
     *         successfully
     */
    ...
    public ResponseEntity<Void> syncPurchaseFigurineTotals(@AuthenticationPrincipal Jwt jwt,
        ...
        log.info("Syncing purchase figurine totals. CollectorId: {}, PurchaseId: {}, CollectionId: {}",
                jwt.getSubject(), purchaseId, collectionId);
    ...
    /**
     * Synchronizes figurine quantities from all purchases associated with a
     * collector collection.
     *
     * <p>
     * This operation recalculates collection quantities based on purchase history
     * records.
     *
     * @param jwt
     *            authenticated collector token
