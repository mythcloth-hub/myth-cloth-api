...
/**
 * Service responsible for managing collector purchase records and their
 * associated figurine items.
 *
 * <p>
 * This service handles the complete lifecycle of collector purchases,
 * including:
 *
 * <ul>
 ...
 *
 * <p>
 * Purchases are tracked separately from the collector's current collection
 * state. This allows preserving purchase history, quantities acquired, prices
 * paid, shipping information, and other transaction details without modifying
 * the current collection inventory.
 *
 * <p>
 * This service validates collector ownership and referenced figurines before
 * performing any persistence operations.
 *
...
public class CollectorPurchaseService {
    ...
    /**
     * Creates a new collector purchase summary together with its figurine line
     * items.
     *
     * <p>
     * This operation creates the purchase record, calculates totals, validates
     * referenced figurines, and persists all line items.
     *
     * @param collectorId
     *            collector identifier
     * @param request
     *            purchase summary data and line items
     * @return the created purchase summary with persisted line items
     * @throws CollectorNotFoundException
     *             when the collector does not exist
     * @throws FigurineNotFoundException
     *             when a referenced figurine does not exist
     */
    @Transactional
    public CollectorPurchaseSummaryLineItemResp createSummaryLineItem(@Positive Long collectorId,
        ...
        List<CollectorPurchaseLineItemResp> lineItems = saveLineItems(savedPurchase, request.lineItems());

        log.info("Created collector purchase summary [{}] with [{}] line items for collector [{}]",
                savedPurchase.getId(), lineItems.size(), collectorId);

        return new CollectorPurchaseSummaryLineItemResp(savedPurchase.getId(), savedPurchase.getOrderDate(),
                savedPurchase.getStore(), savedPurchase.getOrderNumber(), savedPurchase.getCurrency(),
                savedPurchase.getTotalAmount(), savedPurchase.getTotalFigurines(), savedPurchase.getShippingStatus(),
                savedPurchase.getTrackingNumber(), savedPurchase.getCarrier(), savedPurchase.getShippedDate(),
                savedPurchase.getDeliveredDate(), lineItems);
    }

    /**
     * Updates an existing collector purchase summary and its associated figurine
     * line items.
     *
     * <p>
     * If line item information changes, existing items are replaced with the
     * updated values.
     *
     * @param collectorId
     *            collector identifier
     ...
     *            purchase identifier
     * @param request
     *            updated purchase summary data and line items
     ...
     */
    @Transactional
    public CollectorPurchaseSummaryLineItemResp updateSummaryLineItem(@Positive Long collectorId,
            @Positive Long purchaseId, @NotNull @Valid CollectorPurchaseSummaryLineItemReq request) {

        collectorRepository.findById(collectorId).orElseThrow(() -> new CollectorNotFoundException(collectorId));

        CollectorPurchase existing = collectorPurchaseRepository.findByIdAndCollectorId(purchaseId, collectorId)
        ...
        List<CollectorPurchaseFigurine> existingLineItems = collectorPurchaseFigurineRepository
        ...
        if (wasThereAnyChange) {
            List<CollectorPurchaseFigurine> toBeDeleted = collectorPurchaseFigurineRepository
                    .findByPurchase(updatedPurchase);
        ...
        log.info("Updated collector purchase summary [{}] with [{}] line items for collector [{}]",
                updatedPurchase.getId(), lineItems.size(), collectorId);

        return new CollectorPurchaseSummaryLineItemResp(updatedPurchase.getId(), updatedPurchase.getOrderDate(),
                updatedPurchase.getStore(), updatedPurchase.getOrderNumber(), updatedPurchase.getCurrency(),
                updatedPurchase.getTotalAmount(), updatedPurchase.getTotalFigurines(),
                updatedPurchase.getShippingStatus(), updatedPurchase.getTrackingNumber(), updatedPurchase.getCarrier(),
                updatedPurchase.getShippedDate(), updatedPurchase.getDeliveredDate(), lineItems);
    }
     ...
     *
     * <p>
     * Results are ordered by purchase date descending and include associated
     * figurine line items.
     *
     * @param collectorId
     *            collector identifier
    ...
    public List<CollectorPurchaseSummaryLineItemResp> retrieveSummaryLineItems(@Positive Long collectorId) {
        collectorRepository.findById(collectorId).orElseThrow(() -> new CollectorNotFoundException(collectorId));

        List<CollectorPurchase> purchases = collectorPurchaseRepository
                .findByCollectorIdOrderByOrderDateDescIdDesc(collectorId);
        if (purchases.isEmpty()) {
            return List.of();
        }

        List<Long> purchaseIds = purchases.stream().map(CollectorPurchase::getId).toList();
        List<CollectorPurchaseFigurine> allLineItems = collectorPurchaseFigurineRepository
                .findByPurchaseIdIn(purchaseIds);
        ...
        for (CollectorPurchaseFigurine lineItem : allLineItems) {
            ...
            lineItemsByPurchaseId.computeIfAbsent(purchaseId, ignored -> new ArrayList<>())
                    .add(mapToLineItemResponse(lineItem));
        }

        return purchases.stream().map(purchase -> mapToSummaryResponse(purchase,
                lineItemsByPurchaseId.getOrDefault(purchase.getId(), List.of()))).toList();
    }
     ...
     */
    @Transactional(readOnly = true)
    public CollectorPurchaseSummaryLineItemResp retrieveSummaryLineItem(@Positive Long collectorId,
            @Positive Long purchaseId) {
        collectorRepository.findById(collectorId).orElseThrow(() -> new CollectorNotFoundException(collectorId));

        CollectorPurchase purchase = collectorPurchaseRepository.findByIdAndCollectorId(purchaseId, collectorId)
                .orElseThrow(() -> new CollectorPurchaseNotFoundException(purchaseId));

        List<CollectorPurchaseLineItemResp> lineItems = collectorPurchaseFigurineRepository
                .findByPurchaseIdOrderByIdAsc(purchaseId).stream().map(this::mapToLineItemResponse).toList();
    ...
    public void deleteSummaryLineItem(@Positive Long collectorId, @Positive Long purchaseId) {
        ...
        collectorPurchaseFigurineRepository.findByPurchase(purchase)
    ...
    @Transactional
    public void syncPurchaseFigurineTotals(Long collectorId, @Positive Long purchaseId, @Positive Long collectionId) {
        collectorRepository.findById(collectorId).orElseThrow(() -> new CollectorNotFoundException(collectorId));

        var existingPurchase = collectorPurchaseRepository.findByIdAndCollectorId(purchaseId, collectorId)
                .orElseThrow(() -> new CollectorPurchaseNotFoundException(purchaseId));
        ...
        for (CollectorPurchaseFigurine lineItem : lineItems) {
            ...
            // For example, you might want to update some fields in the lineItem or perform
            // calculations.
    ...
    private boolean detectChangesInLineItems(List<CollectorPurchaseFigurine> existingLineItems,
        ...
        for (int i = 0; i < existingLineItems.size(); i++) {
            if (!Objects.equals(existingLineItems.get(i).getFigurine().getId(),
                    collectorPurchaseLineItemReqs.get(i).figurineId())
                    || !Objects.equals(existingLineItems.get(i).getQuantity(),
                            collectorPurchaseLineItemReqs.get(i).quantity())
                    || existingLineItems.get(i).getPricePaid()
                            .compareTo(collectorPurchaseLineItemReqs.get(i).pricePaid()) != 0
                    || existingLineItems.get(i).getPurchaseType() != collectorPurchaseLineItemReqs.get(i)
                            .purchaseType()) {
                return true;
            }
        }
    ...
    private void updatePurchaseCalculatedFields(CollectorPurchase purchase,
            List<CollectorPurchaseLineItemReq> lineItems, ShippingStatus shippingStatus) {
        BigDecimal totalAmount = BigDecimal.ZERO;
        int totalFigurines = 0;
        for (CollectorPurchaseLineItemReq lineItemReq : lineItems) {
            BigDecimal lineTotal = lineItemReq.pricePaid().multiply(BigDecimal.valueOf(lineItemReq.quantity()));
    ...
    private void mapSummaryFields(CollectorPurchaseSummaryLineItemReq request, CollectorPurchase purchase) {
    ...
    private List<CollectorPurchaseLineItemResp> saveLineItems(CollectorPurchase purchase,
            List<CollectorPurchaseLineItemReq> lineItemRequests) {
        ...
        for (CollectorPurchaseLineItemReq lineItemRequest : lineItemRequests) {
            var figurine = figurineRepository.findById(lineItemRequest.figurineId())
            ...
            lineItems.add(new CollectorPurchaseLineItemResp(savedLineItem.getId(), figurine.getId(),
                    savedLineItem.getQuantity(), savedLineItem.getPricePaid(), savedLineItem.getPurchaseType()));
        }
        return lineItems;
    }

    private CollectorPurchaseSummaryLineItemResp mapToSummaryResponse(CollectorPurchase purchase,
            List<CollectorPurchaseLineItemResp> lineItems) {
        return new CollectorPurchaseSummaryLineItemResp(purchase.getId(), purchase.getOrderDate(), purchase.getStore(),
                purchase.getOrderNumber(), purchase.getCurrency(), purchase.getTotalAmount(),
                purchase.getTotalFigurines(), purchase.getShippingStatus(), purchase.getTrackingNumber(),
                purchase.getCarrier(), purchase.getShippedDate(), purchase.getDeliveredDate(), lineItems);
    }

    private CollectorPurchaseLineItemResp mapToLineItemResponse(CollectorPurchaseFigurine lineItem) {
        return new CollectorPurchaseLineItemResp(lineItem.getId(), lineItem.getFigurine().getId(),
                lineItem.getQuantity(), lineItem.getPricePaid(), lineItem.getPurchaseType());
    }
}
