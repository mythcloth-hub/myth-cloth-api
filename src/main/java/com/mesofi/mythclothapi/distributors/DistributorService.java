 ...
 *
 * <p>
 * Provides CRUD operations for distributors, including validation of unique
 * constraints and transformation between request/response DTOs and persistence
 * entities.
 */
@Slf4j
@Service
...
public class DistributorService {
	 ...
	 *
	 * <p>
	 * This method converts the incoming {@link DistributorReq} into a
	 * {@link Distributor}, validates the uniqueness of the distributor by name and
	 * country, and persists it in the repository.
	 *
	 * @param request
	 *            the distributor data to create
	 * @return the created distributor as a {@link DistributorResp}
	 * @throws DistributorAlreadyExistsException
	...
	public DistributorResp createDistributor(DistributorReq request) {
		...
		if (repository.existsByNameAndCountry(entity.getName(), entity.getCountry())) {
			throw new DistributorAlreadyExistsException(request.name().toString(), request.countryCode().toString());
		}
	...
	public DistributorResp retrieveDistributor(Long id) {
		return repository.findById(id).map(mapper::toDistributorResp)
	 ...
	 *
	 * <p>
	 * This method retrieves the existing distributor, validates that updating the
	 * name and country does not conflict with another distributor, and applies the
	 * updates via MapStruct.
	 *
	 * @param id
	 *            the ID of the distributor to update
	 * @param request
	 *            the updated distributor information
	 * @return the updated distributor as a {@link DistributorResp}
	 * @throws DistributorNotFoundException
	 *             if the distributor to update does not exist
	 * @throws DistributorAlreadyExistsException
	 *             if the update results in a name+country conflict with another
	 *             distributor
	 */
	@Transactional
	public DistributorResp updateDistributor(Long id, DistributorReq request) {
		...
		if (!(existing.getName() == entity.getName() && existing.getCountry().equals(entity.getCountry()))) {
			if (repository.existsByNameAndCountry(entity.getName(), entity.getCountry())) {
				throw new DistributorAlreadyExistsException(request.name().toString(),
	 ...
	 *
	 * @param id
	 *            the ID of the distributor to delete
	 * @throws DistributorNotFoundException
	 *             if no distributor with the given ID exists
	 */
