...
/**
 * MapStruct mapper responsible for converting between distributor-related DTOs
 * ({@link DistributorReq}, {@link DistributorResp}) and the {@link Distributor}
 * domain model.
 *
 * <p>
 * Uses MapStruct's code generation to provide efficient, compile-time-safe
 * mappings. Configured as a Spring component.
 */
...
public interface DistributorMapper {
     ...
     *
     * <p>
     * The {@code id} and {@code figurines} fields are ignored, as they are either
     * auto-generated or managed through separate logic.
     *
     * @param request
     *            the incoming distributor creation/update request
     ...
     *
     * <p>
     * Extracts the textual description from the distributor name enum to expose a
     * meaningful string in the API response.
     *
     * @param distributorEntity
     *            the entity to map
     * @return a mapped {@link DistributorResp}
     */
    @Mapping(target = "description", expression = "java(distributorEntity.getName().getDescription())")
    @Mapping(target = "countryCode", source = "country")
    DistributorResp toDistributorResp(Distributor distributorEntity);

    /**
     * Updates an existing {@link Distributor} using the non-null fields provided in
     * a {@link DistributorReq}.
     *
     * <p>
     ...
     * @param entity
     *            the existing entity to apply updates to
     */
    @Mapping(target = "id", ignore = true)
    @Mapping(target = "figurines", ignore = true)
    @Mapping(target = "country", source = "countryCode")
