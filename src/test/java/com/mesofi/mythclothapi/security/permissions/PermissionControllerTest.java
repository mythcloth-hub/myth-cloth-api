...
public class PermissionControllerTest {
    ...
    void createPermission_shouldReturn400_whenRequestBodyIsMissing() throws Exception {
        mockMvc.perform(post("/permissions")).andExpect(status().isBadRequest()).andExpect(jsonPath("$.detail").value(
                ...
                .andExpect(jsonPath("$.instance").value("/permissions")).andExpect(jsonPath("$.status").value("400"))
    ...
    void createPermission_shouldReturn415_whenContentTypeIsMissing() throws Exception {
        ...
        mockMvc.perform(post("/permissions").content("{}")).andExpect(status().isUnsupportedMediaType())
                .andExpect(jsonPath("$.detail").value("Content-Type 'application/octet-stream' is not supported"))
                .andExpect(jsonPath("$.instance").value("/permissions")).andExpect(jsonPath("$.status").value("415"))
    ...
    void createPermission_shouldReturn201AndLocationHeader() throws Exception {
        ...
        mockMvc.perform(post("/permissions").contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.id").value(1L)).andExpect(jsonPath("$.description").value("figurines:read"));
    ...
    void retrievePermission_shouldReturn200_whenPermissionExists() throws Exception {
        ...
        mockMvc.perform(get("/permissions/{id}", 7L)).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(7L))
