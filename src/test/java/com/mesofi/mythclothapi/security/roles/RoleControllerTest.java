...
public class RoleControllerTest {
    ...
    void createRole_shouldReturn400_whenRequestBodyIsMissing() throws Exception {
        mockMvc.perform(post("/roles")).andExpect(status().isBadRequest()).andExpect(jsonPath("$.detail").value(
                ...
                .andExpect(jsonPath("$.instance").value("/roles")).andExpect(jsonPath("$.status").value("400"))
    ...
    void createRole_shouldReturn415_whenContentTypeIsMissing() throws Exception {
        ...
        mockMvc.perform(post("/roles").content("{}")).andExpect(status().isUnsupportedMediaType())
                .andExpect(jsonPath("$.detail").value("Content-Type 'application/octet-stream' is not supported"))
                .andExpect(jsonPath("$.instance").value("/roles")).andExpect(jsonPath("$.status").value("415"))
    ...
    void createRole_shouldReturn400_whenDescriptionExceedsMaxLength() throws Exception {
        ...
        mockMvc.perform(post("/roles").contentType(MediaType.APPLICATION_JSON).content(requestBody))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.detail").value("Your request parameters didn't validate"))
                .andExpect(jsonPath("$.instance").value("/roles")).andExpect(jsonPath("$.status").value("400"))
                .andExpect(jsonPath("$.title").value("Validation Failed")).andExpect(jsonPath("$.timestamp").exists())
                .andExpect(jsonPath("$.errors.description").value("description must not exceed 200 characters"));
    ...
    void createRole_shouldReturn201AndLocationHeader() throws Exception {
        ...
        mockMvc.perform(post("/roles").contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.id").value(1L)).andExpect(jsonPath("$.description").value("Admin"));
    ...
    void retrieveRole_shouldReturn200_whenRoleExists() throws Exception {
        ...
        mockMvc.perform(get("/roles/{id}", 7L)).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(7L))
    ...
    void retrieveRole_shouldReturn200_whenRoleEntriesExist() throws Exception {
        ...
        mockMvc.perform(get("/roles")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2))
                .andExpect(jsonPath("$[0].id").value(1L)).andExpect(jsonPath("$[0].description").value("Admin"))
    ...
    void updateRole_shouldReturn200_whenRoleEntriesExist() throws Exception {
        ...
        mockMvc.perform(put("/roles/{id}", 1L).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.description").value("Admin"));
