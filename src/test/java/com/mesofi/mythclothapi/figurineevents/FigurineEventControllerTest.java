...
class FigurineEventControllerTest {
    ...
    @MockitoBean
    private FigurineEventService service;
    ...
    void createEvent_shouldReturn400_whenRequestBodyIsMissing() throws Exception {
        ...
        mockMvc.perform(post("/figurines/{figurineId}/events", 1L)).andExpect(status().isBadRequest())
    ...
    void createEvent_shouldReturn415_whenContentTypeIsMissing() throws Exception {
        ...
        mockMvc.perform(post("/figurines/{figurineId}/events", 1L).content("{}"))
    ...
    void createEvent_shouldReturn400_whenRequestBodyFailsValidation() throws Exception {
        ...
        mockMvc.perform(
                post("/figurines/{figurineId}/events", 1L).contentType(MediaType.APPLICATION_JSON).content("{}"))
    ...
    void createEvent_shouldReturn201AndDelegateUsingPathFigurineId() throws Exception {
        ...
        mockMvc.perform(post("/figurines/{figurineId}/events", 1L).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated())
                .andExpect(header().string("Location", endsWith("/events/77"))).andExpect(jsonPath("$.id").value(77L))
                .andExpect(jsonPath("$.description").value("Pre-order opened"))
                .andExpect(jsonPath("$.date").value("2020-01-01"));

        verify(service).createFigurineEvent(
                argThat(payload -> payload.getFigurineId() == 1L && "Pre-order opened".equals(payload.getDescription())
    ...
    void retrieveEvent_shouldReturn200_whenEventExists() throws Exception {
        ...
        mockMvc.perform(get("/figurines/{figurineId}/events/{id}", 1L, 15L)).andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(15L)).andExpect(jsonPath("$.date").value("2020-01-01"))
                .andExpect(jsonPath("$.type").value("PREORDER_OPEN")).andExpect(jsonPath("$.region").value("JP"))
                .andExpect(jsonPath("$.description").value("Pre-order opened"));
    ...
    void retrieveEvents_shouldReturnList_whenEventsExist() throws Exception {
        ...
        mockMvc.perform(get("/figurines/{figurineId}/events", 1L)).andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2)).andExpect(jsonPath("$[0].id").value(10L))
                .andExpect(jsonPath("$[0].date").value("2020-01-01"))
                .andExpect(jsonPath("$[0].type").value("PREORDER_OPEN")).andExpect(jsonPath("$[0].region").value("JP"))
                .andExpect(jsonPath("$[0].description").value("Pre-order opened"))
                .andExpect(jsonPath("$[1].id").value(11L)).andExpect(jsonPath("$[1].date").value("2020-01-01"))
                .andExpect(jsonPath("$[1].type").value("PREORDER_OPEN")).andExpect(jsonPath("$[1].region").value("JP"))
    ...
    void updateEvent_shouldReturn200_whenRequestIsValid() throws Exception {
        ...
        mockMvc.perform(put("/figurines/{figurineId}/events/{id}", 1L, 20L).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(20L));

        verify(service).updateFigurineEvent(argThat(figurineId -> figurineId == 1L), argThat(eventId -> eventId == 20L),
                argThat(payload -> payload.getFigurineId() == 1L && "Pre-order opened".equals(payload.getDescription())
                        && LocalDate.of(2020, 1, 1).equals(payload.getDate())));
    }
    ...
    void removeEvent_shouldReturn204_whenRequestIsValid() throws Exception {
        mockMvc.perform(delete("/figurines/{figurineId}/events/{id}", 1L, 5L)).andExpect(status().isNoContent());
    ...
    private FigurineEventResp createEventResponse(long id) {
        return new FigurineEventResp(id, LocalDate.of(2020, 1, 1), true, FigurineEventType.PREORDER_OPEN,
                CountryCode.JP, "Pre-order opened");
    }
}
