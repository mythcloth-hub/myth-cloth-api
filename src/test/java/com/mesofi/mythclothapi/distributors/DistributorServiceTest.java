...
@ActiveProfiles("test")
@SpringBootTest(classes = {DistributorService.class, MapperTestConfig.class, MethodValidationTestConfig.class})
class DistributorServiceTest {
    ...
    void createDistributor_shouldPersistAndReturnMappedResponse_whenRequestIsValid() {
        // Arrange
        DistributorReq request = new DistributorReq(DistributorName.DAM, CountryCode.MX, "https://app.tamashii.mx/");

        Distributor saved = createEntity(1L, DistributorName.DAM, CountryCode.MX, request.website());

        when(distributorRepository.existsByNameAndCountry(DistributorName.DAM, CountryCode.MX)).thenReturn(false);
        when(distributorRepository.save(org.mockito.ArgumentMatchers.any(Distributor.class))).thenReturn(saved);
        ...
        assertThat(response).isNotNull()
                .extracting(DistributorResp::id, DistributorResp::name, DistributorResp::description,
                        DistributorResp::countryCode, DistributorResp::website)
                .containsExactly(1L, DistributorName.DAM.toString(), DistributorName.DAM.getDescription(),
                        CountryCode.MX.toString(), "https://app.tamashii.mx/");
    ...
    void createDistributor_shouldThrowAlreadyExistsException_whenDuplicateNameAndCountry() {
        // Arrange
        DistributorReq request = new DistributorReq(DistributorName.BANDAI, CountryCode.JP, "https://tamashiiweb.com/");

        when(distributorRepository.existsByNameAndCountry(DistributorName.BANDAI, CountryCode.JP)).thenReturn(true);
        ...
        assertThatThrownBy(() -> distributorService.createDistributor(request))
                ...
                .extracting(ex -> ((DistributorAlreadyExistsException) ex).getDetail())
                .isEqualTo("Distributor 'BANDAI' already exists in country 'JP'");

        verify(distributorRepository).existsByNameAndCountry(DistributorName.BANDAI, CountryCode.JP);
        verify(distributorRepository, never()).save(org.mockito.ArgumentMatchers.any(Distributor.class));
    }
    ...
    void retrieveDistributor_shouldThrowNotFoundException_whenDistributorDoesNotExist() {
        ...
        assertThatThrownBy(() -> distributorService.retrieveDistributor(0L))
                .isInstanceOf(DistributorNotFoundException.class).hasMessage("Distributor with id 0 was not found")
                .extracting(ex -> ((DistributorNotFoundException) ex).getId()).isEqualTo(0L);
    ...
    void retrieveDistributor_shouldReturnMappedResponse_whenDistributorExists() {
        ...
        assertThat(response).isNotNull()
                .extracting(DistributorResp::id, DistributorResp::name, DistributorResp::description,
                        DistributorResp::countryCode, DistributorResp::website)
                .containsExactly(5L, DistributorName.BLUE_FIN.toString(), DistributorName.BLUE_FIN.getDescription(),
                        CountryCode.US.toString(), null);
    ...
    void retrieveDistributors_shouldReturnMappedResponses_whenRepositoryReturnsEntities() {
        ...
        Distributor second = createEntity(2L, DistributorName.DS_DISTRIBUTIONS, CountryCode.ES, "https://sdd.com");
    ...
    void updateDistributor_shouldThrowNotFoundException_whenDistributorDoesNotExist() {
        ...
        assertThatThrownBy(() -> distributorService.updateDistributor(77L, request))
                .isInstanceOf(DistributorNotFoundException.class).hasMessage("Distributor with id 77 was not found")
                .extracting(ex -> ((DistributorNotFoundException) ex).getId()).isEqualTo(77L);
    ...
    void updateDistributor_shouldThrowAlreadyExistsException_whenNewNameAndCountryConflict() {
        // Arrange
        DistributorReq request = new DistributorReq(DistributorName.DAM, CountryCode.MX, "https://new.example.com");
        ...
        when(distributorRepository.existsByNameAndCountry(DistributorName.DAM, CountryCode.MX)).thenReturn(true);

        // Act + Assert
        assertThatThrownBy(() -> distributorService.updateDistributor(4L, request))
                .isInstanceOf(DistributorAlreadyExistsException.class).hasMessage("Distributor already exists")
                .extracting(ex -> ((DistributorAlreadyExistsException) ex).getDetail())
                .isEqualTo("Distributor 'DAM' already exists in country 'MX'");
    ...
    void updateDistributor_shouldUpdateAndReturnMappedResponse_whenRequestIsValid() {
        ...
        when(distributorRepository.existsByNameAndCountry(DistributorName.BANDAI, CountryCode.JP)).thenReturn(false);
        ...
        assertThat(response).isNotNull()
                .extracting(DistributorResp::id, DistributorResp::name, DistributorResp::description,
                        DistributorResp::countryCode, DistributorResp::website)
                .containsExactly(3L, DistributorName.BANDAI.toString(), DistributorName.BANDAI.getDescription(),
                        CountryCode.JP.toString(), "https://bandai.com");
    ...
    void updateDistributor_shouldSkipDuplicateCheck_whenNameAndCountryAreUnchanged() {
        // Arrange
        DistributorReq request = new DistributorReq(DistributorName.BANDAI, CountryCode.JP, "https://new-bandai.com");
        Distributor existing = createEntity(6L, DistributorName.BANDAI, CountryCode.JP, "https://old.com");
        ...
        assertThat(response).isNotNull()
                .extracting(DistributorResp::id, DistributorResp::name, DistributorResp::description,
                        DistributorResp::countryCode, DistributorResp::website)
                .containsExactly(6L, DistributorName.BANDAI.toString(), DistributorName.BANDAI.getDescription(),
                        CountryCode.JP.toString(), "https://new-bandai.com");

        verify(distributorRepository).findById(6L);
        verify(distributorRepository, never()).existsByNameAndCountry(DistributorName.BANDAI, CountryCode.JP);
        verify(distributorRepository).save(existing);
    }
    ...
    void updateDistributor_shouldCheckDuplicates_whenOnlyCountryChanges() {
        // Arrange
        DistributorReq request = new DistributorReq(DistributorName.BANDAI, CountryCode.MX, "https://new.example.com");
        Distributor existing = createEntity(7L, DistributorName.BANDAI, CountryCode.JP, "https://old.com");

        when(distributorRepository.findById(7L)).thenReturn(Optional.of(existing));
        when(distributorRepository.existsByNameAndCountry(DistributorName.BANDAI, CountryCode.MX)).thenReturn(true);

        // Act + Assert
        assertThatThrownBy(() -> distributorService.updateDistributor(7L, request))
                .isInstanceOf(DistributorAlreadyExistsException.class).hasMessage("Distributor already exists")
                .extracting(ex -> ((DistributorAlreadyExistsException) ex).getDetail())
                .isEqualTo("Distributor 'BANDAI' already exists in country 'MX'");
    ...
    void removeDistributor_shouldThrowNotFoundException_whenDistributorDoesNotExist() {
        ...
        assertThatThrownBy(() -> distributorService.removeDistributor(8L))
                .isInstanceOf(DistributorNotFoundException.class).hasMessage("Distributor with id 8 was not found")
                .extracting(ex -> ((DistributorNotFoundException) ex).getId()).isEqualTo(8L);
    ...
    private Distributor createEntity(Long id, DistributorName name, CountryCode country, String website) {
