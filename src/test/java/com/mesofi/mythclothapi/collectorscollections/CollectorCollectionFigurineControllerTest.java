...
class CollectorCollectionFigurineControllerTest {
    ...
    @Test
    void addFigurineToCollection_shouldReturnMethodNotAllowed_whenRequestMethodIsInvalid() throws Exception {
        mockMvc.perform(post("/collections").with(jwt().jwt(jwt -> jwt.subject("123"))
                ...
                .andExpect(jsonPath("$.instance").value("/collections")).andExpect(jsonPath("$.status").value("405"))
                .andExpect(jsonPath("$.title").value("Method Not Allowed")).andExpect(jsonPath("$.timestamp").exists());
    ...
    void addFigurineToCollection_shouldReturn404_whenPostingToInvalidEndpoint(String invalidEndpoint) throws Exception {
        mockMvc.perform(post(invalidEndpoint).with(jwt().jwt(jwt -> jwt.subject("123"))
                .authorities(new SimpleGrantedAuthority("collections:figurines:add")))).andExpect(status().isNotFound())
    ...
    void addFigurineToCollection_shouldReturnNoContent_whenValidJwtIsProvided() throws Exception {
        mockMvc.perform(post("/collections/2/figurines/9").with(jwt().jwt(jwt -> jwt.subject("123"))
                .authorities(new SimpleGrantedAuthority("collections:figurines:add"))))
                .andExpect(status().isNoContent());

        AssignFigurinesReq req = new AssignFigurinesReq(List.of(9L), CollectionAssignmentMode.EXISTING, List.of(2L),
                null);
    ...
    void assignFigurinesToCollections_shouldReturnBadRequest_whenBodyIsMissing() throws Exception {
        mockMvc.perform(post("/collections/assign-figurines").with(jwt().jwt(jwt -> jwt.subject("123"))
                .authorities(new SimpleGrantedAuthority("collections:figurines:add"))))
    ...
    void assignFigurinesToCollections_shouldReturn415_whenContentTypeIsMissing() throws Exception {
        mockMvc.perform(post("/collections/assign-figurines").content("{}")
    ...
    @Test
    void assignFigurinesToCollections_shouldReturnBadRequest_whenBodyParametersAreMissing() throws Exception {
        String requestBody = "{}";
    ...
    @Test
    void assignFigurinesToCollections_shouldReturnBadRequest_whenCollectionModeValueIsInvalid() throws Exception {
    ...
    @Test
    void assignFigurinesToCollections_shouldReturnBadRequest_whenFigurineIdsAreEmpty() throws Exception {
    ...
    void assignFigurinesToCollections_shouldAssignFigurines_whenRequestIsValid() throws Exception {
        ...
        AssignFigurinesReq req = new AssignFigurinesReq(List.of(3L), CollectionAssignmentMode.EXISTING, null, null);
    ...
    void retrieveCollectionFigurines_shouldReturnFigurines_whenRequestIsAuthenticated() throws Exception {
        CollectorCollectionFigurineResp resp = new CollectorCollectionFigurineResp(1L, 9L, "Seiya", null, null, null,
                true, 2);
        when(service.retrieveCollectionFigurines(123L, 2L, false, 0, 20)).thenReturn(new PageImpl<>(List.of(resp)));

        mockMvc.perform(get("/collections/2/figurines").contentType(MediaType.APPLICATION_JSON)
                .with(jwt().jwt(jwt -> jwt.subject("123"))
                        .authorities(new SimpleGrantedAuthority("collections:figurines:read"))))
                .andExpect(status().isOk()).andExpect(jsonPath("$.content[0].figurineId").value(9))
        ...
        verify(service).retrieveCollectionFigurines(123L, 2L, false, 0, 20);
    }

    @Test
    void retrieveCollectionFigurine_shouldReturnFigurine_whenRequestIsAuthenticated() throws Exception {
        CollectorCollectionFigurineDetailResp resp = new CollectorCollectionFigurineDetailResp("Seiya", List.of(), null,
                null, null);
        when(service.retrieveCollectionFigurine(123L, 2L, 9L)).thenReturn(resp);

        mockMvc.perform(get("/collections/2/figurines/9").contentType(MediaType.APPLICATION_JSON)
                .with(jwt().jwt(jwt -> jwt.subject("123"))
                        .authorities(new SimpleGrantedAuthority("collections:figurines:read"))))
    ...
    void deleteCollection_shouldReturnNoContent_whenRequestIsAuthenticated() throws Exception {
        mockMvc.perform(delete("/collections/2").with(
                jwt().jwt(jwt -> jwt.subject("123")).authorities(new SimpleGrantedAuthority("collections:delete"))))
                .andExpect(status().isNoContent());
    ...
    @Test
    void duplicateCollection_shouldReturnCreatedWithLocation_whenRequestIsAuthenticated() throws Exception {
