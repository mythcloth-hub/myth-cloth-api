...
class FigurineImageControllerTest {
	...
	@MockitoBean
	private FigurineImageService service;
	...
	void createImage_shouldReturn400_whenRequestBodyIsMissing() throws Exception {
		...
		mockMvc.perform(post("/figurines/{figurineId}/images", 1L)).andExpect(status().isBadRequest())
	...
	void createImage_shouldReturn415_whenContentTypeIsMissing() throws Exception {
		...
		mockMvc.perform(post("/figurines/{figurineId}/images", 1L).content("{}"))
	...
	void createImage_shouldReturn400_whenRequestBodyFailsValidation() throws Exception {
		...
		mockMvc.perform(
				post("/figurines/{figurineId}/images", 1L).contentType(MediaType.APPLICATION_JSON).content("{}"))
	...
	void createImage_shouldReturn201AndUsePathFigurineId_whenRequestIsValid() throws Exception {
		...
		FigurineImageResp response = new FigurineImageResp(List.of("https://images.example/pegasus.jpg"));
		when(service.createFigurineImage(any())).thenReturn(response);

		mockMvc.perform(post("/figurines/{figurineId}/images", 1L).contentType(MediaType.APPLICATION_JSON)
		...
		assertThat(captor.getValue().getImageUrl()).isEqualTo(URI.create("https://images.example/pegasus.jpg"));
	...
	@Test
	void retrieveImages_shouldReturn200AndUseDefaultOfficialFlag_whenParamIsMissing() throws Exception {
		FigurineImageResp response = new FigurineImageResp(List.of("https://images.example/official.jpg"));
		when(service.retrieveFigurineImages(1L, true)).thenReturn(response);

		mockMvc.perform(get("/figurines/{figurineId}/images", 1L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.officialImageUrls.length()").value(1))
	...
	void retrieveImages_shouldReturn200_whenOfficialFlagIsFalse() throws Exception {
		...
		mockMvc.perform(get("/figurines/{figurineId}/images", 1L).param("isOfficialImage", "false"))
				.andExpect(status().isOk()).andExpect(jsonPath("$.officialImageUrls.length()").value(1))
	...
	void removeImage_shouldReturn204_whenRequestIsValid() throws Exception {
		mockMvc.perform(delete("/figurines/{figurineId}/images", 1L)
				.param("imageUrl", "https://images.example/pegasus.jpg").param("isOfficialImage", "false"))
				.andExpect(status().isNoContent());

		verify(service).removeFigurineImage(1L, URI.create("https://images.example/pegasus.jpg"), false);
	}
}
