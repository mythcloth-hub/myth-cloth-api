...
class StatisticsControllerTest {
    ...
    void retrieveStatistics_shouldReturnAggregatedValues() throws Exception {
        when(service.retrieveStatistics(any())).thenReturn(new StatisticsResp(2, Map.of("Myth Cloth EX", 2),
                Map.of("Hades", 2), Map.of(), Map.of(), Map.of("RELEASED", 1, "ANNOUNCED", 1)));

        mockMvc.perform(get("/stats").param("name", "  Gemini Saga  ").param("lineUpId", "10").param("releaseStatus",
                "RELEASED")).andExpect(status().isOk()).andExpect(jsonPath("$.totalFigurines").value(2))
    ...
    void retrieveYearlyReleasePrices_shouldReturnYearlyPriceList() throws Exception {
        ...
        mockMvc.perform(get("/stats/prices/releases/years").param("name", " Saga ").param("releaseStatus", "RELEASED"))
                .andExpect(status().isOk()).andExpect(jsonPath("$[0].year").value(2025))
