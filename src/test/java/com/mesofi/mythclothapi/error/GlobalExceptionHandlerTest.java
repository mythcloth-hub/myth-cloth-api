...
class GlobalExceptionHandlerTest {
	...
	void handleEnumConversionError_shouldIncludeAllowedValues_whenRequiredTypeIsEnum() {
		...
		assertThat(result.getProperties()).containsEntry("error",
	...
	void handleValidationErrors_shouldReturnErrorsMap() {
		...
		when(bindingResult.getFieldErrors())
				.thenReturn(List.of(new FieldError("request", "name", "name must not be blank"),
		...
		assertThat(result.getProperties().get("errors"))
				.isEqualTo(Map.of("name", "name must not be blank", "countryCode", "countryCode is required"));
	}

	@Test
	void handleApiExceptions_shouldReturnProblemFromExceptionData() {
		ProblemDetail alreadyExists = handler
	...
	void handleIntegrationException_shouldUseExceptionMessageForTitleAndDetail() {
		IntegrationException ex = new IntegrationException(ServiceName.FACEBOOK, HttpStatus.BAD_GATEWAY,
				"Facebook gateway error");
