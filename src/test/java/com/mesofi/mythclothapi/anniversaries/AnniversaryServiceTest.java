...
public class AnniversaryServiceTest {
    ...
    void createAnniversary_shouldPersistAndReturnMappedResponse_whenRequestIsValid() {
        // Arrange
        AnniversaryReq request = request("Saint Seiya 20th Anniversary", 20, AnniversaryType.SAINT_SEIYA);

        when(anniversaryRepository.save(any(Anniversary.class))).thenAnswer(invocation -> {
        ...
        // Assert
        assertThat(response)
                .isEqualTo(new AnniversaryResp(1L, "Saint Seiya 20th Anniversary", 20, AnniversaryType.SAINT_SEIYA));
    ...
    void retrieveAnniversary_shouldReturnMappedResponse_whenAnniversaryExists() {
        ...
        assertThat(response).isEqualTo(
                new AnniversaryResp(7L, "Hades Chapter Celebration", 15, AnniversaryType.TAMASHII_NATIONS_WORLD_TOUR));
    ...
    void retrieveAnniversaries_shouldReturnMappedResponses_whenRepositoryReturnsEntities() {
        ...
        when(anniversaryRepository.findAll(Sort.by("id")))
                .thenReturn(List.of(anniversary(1L, "Classic Anime Anniversary", 10, AnniversaryType.SAINT_CLOTH_MYTH),
        ...
        assertThat(responses).containsExactly(
                new AnniversaryResp(1L, "Classic Anime Anniversary", 10, AnniversaryType.SAINT_CLOTH_MYTH),
    ...
    void updateAnniversary_shouldUpdateExistingEntityAndReturnMappedResponse_whenRequestIsValid() {
        ...
        AnniversaryReq request = request("Updated Anniversary", 25, AnniversaryType.TAMASHII_NATIONS_WORLD_TOUR);

        when(anniversaryRepository.findById(3L)).thenReturn(Optional.of(existing));
        when(anniversaryRepository.save(any(Anniversary.class))).thenAnswer(invocation -> invocation.getArgument(0));
        ...
        // Assert
        assertThat(response).isEqualTo(
                new AnniversaryResp(3L, "Updated Anniversary", 25, AnniversaryType.TAMASHII_NATIONS_WORLD_TOUR));
