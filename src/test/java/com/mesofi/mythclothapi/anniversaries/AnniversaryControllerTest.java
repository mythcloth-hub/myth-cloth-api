...
class AnniversaryControllerTest {
	...
	void createAnniversary_shouldReturn400_whenRequestBodyIsMissing() throws Exception {
		...
		mockMvc.perform(post("/anniversaries")).andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.detail")
						.value("Required request body is missing: public org.springframework.http.ResponseEntity"
				...
				.andExpect(jsonPath("$.instance").value("/anniversaries")).andExpect(jsonPath("$.status").value("400"))
	...
	void createAnniversary_shouldReturn415_whenContentTypeIsMissing() throws Exception {
		...
		mockMvc.perform(post("/anniversaries").content("{}")).andExpect(status().isUnsupportedMediaType())
				.andExpect(jsonPath("$.detail").value("Content-Type 'application/octet-stream' is not supported"))
				.andExpect(jsonPath("$.instance").value("/anniversaries")).andExpect(jsonPath("$.status").value("415"))
	...
	void createAnniversary_shouldReturn400_whenDescriptionExceedsMaxLength() throws Exception {
		...
		mockMvc.perform(post("/anniversaries").contentType(MediaType.APPLICATION_JSON).content(requestBody))
	...
	void createAnniversary_shouldReturn201AndLocationHeader() throws Exception {
		...
		mockMvc.perform(post("/anniversaries").contentType(MediaType.APPLICATION_JSON)
	...
	void retrieveAnniversary_shouldReturn200_whenAnniversaryExists() throws Exception {
		...
		mockMvc.perform(get("/anniversaries/{id}", 1L)).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1L))
				.andExpect(jsonPath("$.description").value("Saint Seiya 35th Anniversary"))
				.andExpect(jsonPath("$.year").value(35));
	...
	void retrieveAnniversaries_shouldReturnList_whenAnniversariesExist() throws Exception {
		...
		mockMvc.perform(get("/anniversaries")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2))
	...
	void updateAnniversary_shouldReturn200_whenRequestIsValid() throws Exception {
		...
		mockMvc.perform(put("/anniversaries/{id}", 1L).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(1L)).andExpect(jsonPath("$.description").value("Updated Anniversary"))
