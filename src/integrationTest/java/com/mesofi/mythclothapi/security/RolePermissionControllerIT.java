...
public class RolePermissionControllerIT extends ControllerBaseIT {
	...
	void fullRolePermissionFlow() {
		...
		if (availableRoles.isEmpty()) {
			// Create roles and permissions if they don't exist. This is to ensure the test
			// can be run
		...
		syncPermissions(idRoleAdmin, idPermission5, idPermission2); // add idPermission5 and keep idPermission2, remove
	...
	private List<RoleResp> getAllRoles() {
		ResponseEntity<RoleResp[]> response = rest.get().uri(ROLE).retrieve().toEntity(RoleResp[].class);
	...
	private List<PermissionResp> getAllPermissions(int expectedNumberOfPermissions) {
		ResponseEntity<PermissionResp[]> response = rest.get().uri(PERMISSION).retrieve()
				.toEntity(PermissionResp[].class);
	...
	private List<PermissionResp> getPermissionByRole(Long idRole, int expectedNumberOfPermissions) {
		ResponseEntity<PermissionResp[]> response = rest.get().uri(ROLE_PERMISSION, idRole).retrieve()
				.toEntity(PermissionResp[].class);
	...
	private Long createRole(String description) {
		...
		ResponseEntity<RoleResp> response = rest.post().uri(ROLE).body(request).retrieve().toEntity(RoleResp.class);
	...
	private Long createPermission(String description) {
		...
		ResponseEntity<PermissionResp> response = rest.post().uri(PERMISSION).body(request).retrieve()
				.toEntity(PermissionResp.class);
