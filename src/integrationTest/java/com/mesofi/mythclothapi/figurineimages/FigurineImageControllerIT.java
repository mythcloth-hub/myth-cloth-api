...
public class FigurineImageControllerIT extends ControllerBaseIT {
    ...
    void fullCrudFigurineImagesFlow() {
        ...
        // CREATE
        List<String> images1 = createFigurineImage(figIds.id(),
                URI.create("https://imagizer.imageshack.com/img924/4647/beyAuJ.jpg"), true);

        List<String> images2 = createFigurineImage(figIds.id(),
                URI.create("https://imagizer.imageshack.com/img924/4647/Es2hoO.jpg"), true);

        List<String> images3 = createFigurineImage(figIds.id(),
                URI.create("https://imagizer.imageshack.com/img924/4647/GzzH08.jpg"), true);
        ...
        // DELETE
        removeResource(rest, IMAGES_BY_FIGURINE + "?imageUrl={imageUrl}&isOfficialImage={official}", figIds.id(),
                "https://imagizer.imageshack.com/img924/4647/beyAuJ.jpg", true);
        removeResource(rest, IMAGES_BY_FIGURINE + "?imageUrl={imageUrl}&isOfficialImage={official}", figIds.id(),
                "https://imagizer.imageshack.com/img924/4647/Es2hoO.jpg", true);
        removeResource(rest, IMAGES_BY_FIGURINE + "?imageUrl={imageUrl}&isOfficialImage={official}", figIds.id(),
                "https://imagizer.imageshack.com/img924/4647/GzzH08.jpg", true);
    ...
    private List<String> readFigurineImages(Long figurineId) {
        ResponseEntity<FigurineImageResp> response = rest.get().uri(IMAGES_BY_FIGURINE, figurineId).retrieve()
    ...
    private List<String> createFigurineImage(Long figurineId, URI uri, boolean officialImage) {
        ...
        ResponseEntity<FigurineImageResp> response = rest.post().uri(IMAGES_BY_FIGURINE, figurineId)
                .body(figurineImageReq).retrieve().toEntity(FigurineImageResp.class);
